...
const PROMPTS = {
  wie_sagt_man: (avoid, level) => `Erstelle 15 Beispiele für das Spiel "Wie sagt man das?" (Niveau: ${level}).
...
${avoid}
Antworte als JSON-Array mit 15 Objekten, jedes mit genau diesen Feldern:
...
- "formality": eine von "formal", "neutral", "informal"`,
  ...
  passt_das: (avoid, level) => `Erstelle 15 Beispiele für das Spiel "Passt das?" (Niveau: ${level}).
...
- "explanation_he": kurze Erklärung auf Hebräisch (1-2 Sätze), wann/warum man das so sagt`,
  ...
  ergaenze: (avoid, level) => `Erstelle 15 Beispiele für das Spiel "Ergänze den Satz" (Niveau: ${level}).
...
- "explanation_he": kurze Erklärung auf Hebräisch (1-2 Sätze), warum diese Fortsetzung am natürlichsten klingt`,
  ...
  kenn_ich_das: (avoid, level) => `Erstelle 15 Beispiele für das Spiel "Kenn ich das?" (Niveau: ${level}) — Begriff erraten.
...
- "explanation_he": kurze zusätzliche Erklärung auf Hebräisch, wie/wann man den Begriff verwendet`,
  ...
  schnell_runde: (avoid, level) => `Erstelle 15 Beispiele für das Spiel "Schnell-Runde" (Niveau: ${level}) — Zeitdruck-Quiz.
