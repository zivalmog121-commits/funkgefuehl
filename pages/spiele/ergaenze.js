...
export default function Erganze() {
  ...
  function next() {
    ...
    if (index + 1 < items.length) {
      ...
      const { xpGain } = recordSession("ergaenze", newResults);
      setResults(newResults);
      setXpGain(xpGain);
      setDone(true);
    }
  ...
  function collect() {
    const item = items[index];
    const full = `${item.starter} ${item.options[item.correctIndex]}`;
    addToCollection({
      term: full,
  ...
  if (error) {
    return (
      <Layout title="Ergänze den Satz" onBack={() => router.push("/")}>
  ...
  return (
    <Layout title="Ergänze den Satz" onBack={() => router.push("/")}>
      ...
      <div className="prompt-card">
        <div className="prompt-label">Wie geht's am natürlichsten weiter?</div>
        <div className="prompt-main">„{item.starter}"</div>
      </div>
