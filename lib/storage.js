...
export function saveState(state) {
  ...
  try {
    // Only sync if we're in a browser and auth module is available
    if (typeof window !== "undefined" && window.__firebaseSync) {
      window.__firebaseSync(state).catch(err => {
        console.error("Firebase sync failed:", err);
      });
    }
  } catch (err) {
    // Sync error - app still works with localStorage
