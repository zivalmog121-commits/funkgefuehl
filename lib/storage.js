...
import { syncStateToFirebase } from './firebaseSync';
import { auth } from './firebase';

// Helper to save state with Firebase sync
export function saveState(newState) {
  localStorage.setItem('funkgefuehl:state', JSON.stringify(newState));
  
  // Sync to Firebase if user is signed in
  if (auth?.currentUser) {
    syncStateToFirebase(newState).catch(err => {
